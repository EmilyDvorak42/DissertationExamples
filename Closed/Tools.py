...
ii_cutter = 0.8
it_cutter = 0.9
def Grouper(geo,reco,icconfig,itconfig,typer):
	filtered = False
	calculator = phys_services.I3ScaleCalculator(geo, icconfig, itconfig)
	scale_inice = calculator.scale_inice(reco)
	scale_icetop = calculator.scale_icetop(reco)
	if typer == "G1":
		if ((scale_inice < ii_cutter) and (scale_icetop <= it_cutter)):
			filtered = True
	if typer == "G2":
		if ((scale_inice < ii_cutter) and (scale_icetop > it_cutter)):
			filtered = True
